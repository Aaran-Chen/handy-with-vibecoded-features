...
unsafe extern "system" fn single_instance_window_proc<R: Runtime>(
    ...
    match msg {
        ...
        // HANDY FORK PATCH: ignore WM_CLOSE. This helper window is WS_VISIBLE
        // (layered/transparent) and can surface in the taskbar after an
        // explorer.exe restart (see the comment in create_event_target_window)
        // — if anything then closes it, DefWindowProcW destroys it and
        // single-instance detection silently breaks: every later launch
        // becomes a duplicate instance with its own keyboard hooks. The
        // window must only die with the process (or via destroy()).
        w32wm::WM_CLOSE => 0,
